...
const VALIDATORS: Record<string, z.ZodSchema> = {
  makeUser: z.never({ message: 'TODO' }),
  updateUser: z.never({ message: 'TODO' }),
  getUser: z.never({ message: 'TODO' }),
  makeChatRoom: z.never({ message: 'TODO' }),
  getChatRoom: z.never({ message: 'TODO' }),
  makeChatMsg: z.never({ message: 'TODO' }),
  findChatMsgs: z.never({ message: 'TODO' }),
};

export function validate<T>(command: keyof typeof VALIDATORS,
