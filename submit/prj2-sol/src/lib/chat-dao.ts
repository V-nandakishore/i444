...
type ChatMsg1 = Omit<T.ChatMsg, 'chatName'|'roomName'> &
  { userId: T.UserIdX, roomId: T.RoomIdX, };
...
type DbQuery = { _id?: T.MsgIdX, userId?: T.UserIdX, roomId: T.RoomIdX
	       $text?: { $search: string },
...
export class ChatDao {
  ...
  async makeUser(user: T.RawUser) : Promise<E.Result<T.UserIdX, E.Errs>>
  {
	...
	return E.errResult(E.errs(`user email or chatName already exists`,
				 'E_EXISTS'));
      }
      else {
	return E.errResult(E.errs((e as Error).message, 'E_DB'));
   ...
   *    E_EXISTS:  chatName or email already exists.
   */
  async updateUser(query: {id: T.UserIdX} | {email: T.EmailX} |
                  {chatName: T.NameX}, updates: Partial<T.RawUser>)
    : Promise<E.Result<T.User, E.Errs>> 
  {
    try {
      ...
      const result =
	await this.users.findOneAndUpdate(query1, {$set: updates1}, options);
      if (result !== null) {
   ...
   *  any).
   */
  private async findChatMsgsLo(search: FindParams1)
